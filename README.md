...
1. Запускаем скрипт ```scripts/setup-ws.ps1```
2. Запускает **только инфраструктуру** (Базы данных, Kafka, Keycloak, ELK). Микросервисы вы запускаете локально через вашу IDE.
```bash
docker compose -f docker-compose-dev.yml up -d
```

3. Запуск сервисов по отдельности в корне каждого:
```bash
./gradlew bootRun
```
